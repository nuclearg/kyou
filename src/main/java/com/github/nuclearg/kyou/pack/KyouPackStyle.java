 ...
 * 组包样式
 * <p>
 * {@link KyouPackStyle}定义了将数据变成实际的二进制的报文的过程。其中定义了若干个{@link StyleUnit}(组包样式单元)，描述了针对某些报文元素采用的组包过程。<br/>
 * 需要定义一个针对根元素("#")的{@link StyleUnit}，该{@link StyleUnit}是整个组包过程的入口点。<br/>
 * 实际的组包过程大致如下：<br/>
 * <li>1. 先找到报文中的根节点</li>
 * <li>2. 由上至下遍历{@link KyouPackStyle}，找到第一个与之匹配的{@link StyleUnit}</li>
 * <li>3. 使用这个{@link StyleUnit}中定义的格式对报文数据进行组包。如果这个{@link StyleUnit}引用了其它元素（例如m参数，其表示对自身的各个子节点进行组包）则拿到这些元素，递归重复2和3。</li>
 * </p>
 * 
 * @author ng
 */
public class KyouPackStyle {
    ...
    public KyouPackStyle(InputSource in) {
        try {
            ...
            List<StyleUnit> styles = new ArrayList<>();
            for (Element e : styleElements)
                styles.add(new StyleUnit(e, this));
